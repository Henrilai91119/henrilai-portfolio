...
## 📅 更新日誌 (Changelog)
- **2026-05-31**: 徹底修復網址同步與深層連結 (Deep Linking) 邏輯。
    - 實作原子化導航 `navigateTo`，解決分類切換時產生無效路徑的問題。
    - 修復中文專案名稱亂碼問題，複製連結時自動產出可讀的中文字串。
    - 強化 `parseHash` 優先權，解決外部連結進入時會被自動重置回首頁或第一個項目的問題。
    - **程式碼優化**：建立 `getCurrentHash` 統一網址狀態邏輯，移除重複程式碼並提升維護性。
- **2026-04-27**: 實作圖片防護功能 (禁止右鍵、禁止拖拽、禁止行動端長按選單)。
- **2026-04-27**: 專案儲存庫正式遷移至 GitHub。
