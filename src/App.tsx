...
import { Instagram, X, ArrowLeft, ChevronLeft, ChevronRight, Moon, Sun, Youtube, ArrowUp, Link, Check } from 'lucide-react';
...
function App() {
  ...
  const isParsingHash = useRef(false);

  // Centralized navigation to ensure atomic state updates
  const navigateTo = (category: string, project: string | null = null, subProject: string | null = null, year: string | null = null) => {
  ...
  const parseHash = () => {
    ...
    isParsingHash.current = true;
    const parts = rawHash.split('/').map(part => {
    ...
    setTimeout(() => { isParsingHash.current = false; }, 100);
  };

  // Initial parse and listen for hash changes
  useEffect(() => {
    parseHash();
  ...
  }, [activeCategory, selectedProject, selectedSubProject, activeYear]);
  ...
  useEffect(() => {
    ...
    // Check if the current URL hash already specifies a valid project or year
    const rawHash = window.location.hash.replace('#', '');
    const parts = rawHash.split('/').map(p => {
      try { return decodeURIComponent(p); } catch (e) { return p; }
    });
    
    // If the hash matches the current category AND specifies a project/year, 
    // ABORT the default reset/selection logic.
    const hasValidSelectionInHash = parts[0] === activeCategory && parts[1];
    if (hasValidSelectionInHash) return;

    if (isInitialMount.current) {
      isInitialMount.current = false;
      if (window.location.hash && window.location.hash !== '#Moments%20in%20Time') return;
    }

    // Default selection logic (only runs when no specific project is in URL)
    ...
    } else if (activeCategory === 'Commissioned') {
      // In Commissioned, we don't necessarily want to jump into the first project, 
      // we usually want to show the project list (selectedProject = null).
  ...
  return (
    <div 
      className="min-h-screen bg-white dark:bg-[#0a0a0a] font-sans text-black dark:text-white selection:bg-black selection:text-white dark:selection:bg-white dark:selection:text-black transition-colors duration-500"
    ...
    >
      ...
      <main className={`lg:ml-64 ${isSeamlessLayout ? 'p-0' : 'p-8 md:p-12 lg:p-16 lg:pt-12'}`}>
        <AnimatePresence mode="wait">
          ...
          >
            {activeCategory === 'BIO' ? (
              <div className="max-w-xl mx-auto lg:mx-0 p-8 relative">
                <button onClick={handleCopyLink} className="absolute top-8 right-8 text-gray-300 hover:text-black dark:hover:text-white transition-colors p-1 z-10" title="Copy Link">
              ...
              <div className="max-w-5xl mx-auto p-8 space-y-32 py-20 relative">
                <button onClick={handleCopyLink} className="absolute top-0 right-8 text-gray-300 hover:text-black dark:hover:text-white transition-colors p-1 z-10" title="Copy Link">
                  {copied ? <Check size={14} className="text-green-500" /> : <Link size={14} />}
                </button>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-12 gap-y-24 px-4 md:px-8">
                {Array.from(new Set(currentCategoryItems.map(i => i.title)))
                  ...
                  .map(title => {
                    ...
                    return (
                      <div key={title} onClick={() => navigateTo('Commissioned', title)} className="group cursor-pointer flex flex-col items-center text-center px-4 md:px-8">
              ...
              <div className="space-y-12">
                <header className="sticky top-0 z-20 bg-white/90 dark:bg-[#0a0a0a]/90 backdrop-blur-md py-6 mb-16 border-b border-gray-50 dark:border-gray-900 space-y-6">
                  {activeCategory === 'Moments in Time' ? (
                    <nav className="flex justify-start md:justify-center items-center space-x-8 md:space-x-12 px-8 overflow-x-auto no-scrollbar scroll-smooth">
                      {allYears.map(year => (
                        <button key={year} onClick={() => navigateTo('Moments in Time', null, null, year)} className={`text-[0.62rem] uppercase tracking-[0.4em] transition-all duration-500 whitespace-nowrap ${activeYear === year ? 'text-black dark:text-white font-bold border-b border-black dark:border-white pb-1' : 'text-gray-300 hover:text-black dark:hover:text-white'}`}>{year}</button>
                      ))}
                      <button onClick={handleCopyLink} className="text-gray-300 hover:text-black dark:hover:text-white transition-colors ml-4 p-1 shrink-0" title="Copy Link">
                      ...
                      {activeCategory === 'Design' && (
                        <nav className="flex justify-start md:justify-center space-x-8 md:space-x-12 px-8 overflow-x-auto no-scrollbar">
                          {Array.from(new Set(currentCategoryItems.map(i => i.title))).sort().map(cat => (
                            <button key={cat} onClick={() => navigateTo('Design', cat)} className={`text-[0.62rem] uppercase tracking-[0.4em] transition-all duration-500 whitespace-nowrap ${selectedProject === cat ? 'text-black dark:text-white font-bold' : 'text-gray-300 hover:text-black dark:hover:text-white'}`}>{cat}</button>
                          ))}
                        </nav>
                      )}
                      {selectedProject && (
                        <div className="px-8 flex items-center justify-between">
                          <button onClick={() => navigateTo(activeCategory, null)} className="flex items-center text-[0.62rem] uppercase tracking-[0.3em] text-gray-400 hover:text-black dark:hover:text-white group">
                          ...
                          <div className="flex items-center space-x-4">
                            ...
                            <button onClick={handleCopyLink} className="text-gray-300 hover:text-black dark:hover:text-white transition-colors p-1" title="Copy Link">
                              {copied ? <Check size={14} className="text-green-500" /> : <Link size={14} />}
                            </button>
                          </div>
                        </div>
                      ...
                      {subProjectList.length > 0 && (
                        <nav className="flex justify-start md:justify-center space-x-6 md:space-x-8 px-8 overflow-x-auto no-scrollbar pt-2">
                          {subProjectList.map(sub => (
                            <button key={sub} onClick={() => navigateTo(activeCategory, selectedProject, sub)} className={`text-[0.56rem] uppercase tracking-[0.3em] transition-all duration-500 whitespace-nowrap ${selectedSubProject === sub ? 'text-black dark:text-white border-b border-black dark:border-white' : 'text-gray-300 hover:text-black dark:hover:text-white'}`}>{sub}</button>
                          ))}
                        </nav>
                      )}
                    </>
