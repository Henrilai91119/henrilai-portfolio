  ...
  return (
    <div className={`relative bg-transparent overflow-hidden ${className}`}>
      <motion.img
        ...
        {...props}
      />
      {/* Year Label Overlay - Reserved for future use if needed */}
    </div>
  );
};
    ...
    <div className="min-h-screen bg-white selection:bg-black selection:text-white font-sans text-black">
      ...
      <main className={`lg:ml-64 ${isSeamlessLayout ? 'p-0' : 'p-8 md:p-12 lg:p-16 lg:pt-12'} text-black`}>
        {activeCategory === 'BIO' ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full h-[60vh] flex flex-col items-center justify-center text-center text-black">
            <p className="text-[0.62rem] uppercase tracking-[0.5em] text-gray-300">Section under construction</p>
            <h2 className="text-[0.85rem] font-bold tracking-[0.3em] uppercase text-black mt-4">正在建置中</h2>
          </motion.div>
          ...
          <div className={`${isSeamlessLayout ? 'w-full text-black' : 'space-y-12 text-black'}`}>
            ...
            {currentDescription && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`max-w-2xl mx-auto mb-24 px-8 ${isSeamlessLayout ? 'mt-32 text-black text-black' : ''} text-black text-black text-black text-black`}>
                <div className="text-[0.8rem] leading-[2.2] text-gray-500 tracking-wide font-light whitespace-pre-wrap text-center italic text-black font-sans text-black text-black text-black">{currentDescription}</div>
              </motion.div>
            )}
            {activeCategory === 'Moments in Time' ? (
              <div className="space-y-48 text-black text-black text-black">
                {groupedVisibleItems.map(([year, items]) => (
                  <section key={year} ref={el => yearRefs.current[year] = el} className="space-y-16 text-black text-black text-black text-black">
                    <header className="border-b border-gray-100 pb-6 mb-12 ml-8 md:ml-12 text-black text-[0.875rem] text-black text-black text-black"><h2 className="text-[0.875rem] font-bold tracking-[0.6em] text-black/30 uppercase italic text-black font-sans text-black text-black text-black text-black">{year}</h2></header>
                    <div className="columns-1 sm:columns-2 md:columns-3 gap-16 lg:gap-24 space-y-16 lg:space-y-24 text-black text-black text-black text-black text-black text-black">
                      {items.map((item, index) => (<div key={item.id} onClick={() => setSelectedImage(item)} className="break-inside-avoid mb-16 lg:mb-24 group cursor-crosshair px-4 md:px-8 lg:px-12 text-black text-black text-black text-black"><LazyImage src={item.imageUrl} alt={`${year} work ${index + 1}`} priority={index < 6} showYear={false} imgClassName="h-auto transition-transform duration-1000 ease-out group-hover:scale-[1.01] text-black text-black text-black text-black text-black" /></div>))}
                    </div>
                  </section>
                ))}
              </div>
            ) : (
              <div className={isSeamlessLayout ? 'flex flex-col w-full max-w-2xl mx-auto text-black text-black text-black text-black' : 'columns-1 sm:columns-2 md:columns-3 gap-16 lg:gap-24 space-y-16 lg:space-y-24 text-black text-black text-black text-black'}>
                {displayItems.map((item, index) => (
                  <div key={item.id} onClick={() => setSelectedImage(item)} className={isSeamlessLayout ? 'w-full text-black text-black text-black text-black' : 'break-inside-avoid mb-16 lg:mb-24 group cursor-crosshair px-4 md:px-8 lg:px-12 text-black text-black text-black text-black'}>
                    <LazyImage src={item.imageUrl} alt={item.title || 'Portfolio Work'} priority={index < 6} showYear={false} imgClassName="h-auto w-full block" className={isSeamlessLayout ? 'bg-transparent text-black text-black text-black' : 'text-black text-black text-black text-black'} />
                    {(!selectedProject && activeCategory !== 'Moments in Time') && <div className="mt-6 opacity-0 group-hover:opacity-100 transition-opacity duration-500 text-right text-black font-sans text-black text-black text-black text-black"><p className="text-[0.56rem] uppercase tracking-[0.3em] text-gray-300 font-light text-black font-sans text-black text-black text-black text-black text-black">{item.title}</p></div>}
                  </div>
                ))}
              </div>
            )}
      ...
      <AnimatePresence>
        {selectedImage && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-white/98 p-4 md:p-12 lg:p-24 cursor-zoom-out text-black text-black text-black text-black text-black text-black" onClick={() => setSelectedImage(null)}>
            <button className="absolute top-8 right-8 text-black hover:rotate-90 transition-transform duration-500 p-2 text-black text-black text-black text-black text-black text-black text-black text-black"><X size={24} strokeWidth={1} /></button>
            <button className="absolute left-8 top-1/2 -translate-y-1/2 text-gray-300 hover:text-black transition-colors p-4 z-50" onClick={(e) => { e.stopPropagation(); navigateImage('prev'); }}><ChevronLeft size={48} strokeWidth={1} /></button>
            <button className="absolute right-8 top-1/2 -translate-y-1/2 text-gray-300 hover:text-black transition-colors p-4 z-50" onClick={(e) => { e.stopPropagation(); navigateImage('next'); }}><ChevronRight size={48} strokeWidth={1} /></button>
            <motion.img key={selectedImage.id} initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }} transition={{ type: "spring", damping: 30, stiffness: 200 }} src={selectedImage.imageUrl} alt={selectedImage.title} className="max-w-full max-h-full object-contain shadow-2xl text-black text-black" />
            <div className="absolute bottom-12 left-12 text-left text-black text-black text-black text-black text-black text-black text-black text-black text-black text-black">
              <p className="text-[0.56rem] uppercase tracking-[0.5em] text-gray-300 font-light text-black font-sans text-black text-black text-black text-black text-black text-black text-black text-black text-black text-black text-black text-black text-black text-black">{selectedImage.title} {selectedImage.subTitle ? `— ${selectedImage.subTitle}` : ''} <span className="ml-4 opacity-50 tracking-widest text-black text-black text-black text-black text-black text-black text-black text-black text-black text-black">{selectedImage.imageUrl?.match(/\d{4}/)?.[0]}</span></p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
